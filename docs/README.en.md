...
- **Gemini Integrations** (`app/utils.call_gemini_api`):
  ...
  - `/api/agent/analytics-report`: Summarises cohort + cost-benefit results into actionable Markdown recommendations with KPI callouts. Requires the Gemini credential in the `X-Api-Key` header.
