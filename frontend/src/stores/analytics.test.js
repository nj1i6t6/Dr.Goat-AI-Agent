...
describe('analytics store', () => {
  ...
  it('stores report output', async () => {
    const store = useAnalyticsStore()
    await store.generateReport({ filters: {} }, 'key')
    expect(store.reportState.html).toContain('<p>ok</p>')
    expect(apiMock.generateAnalyticsReport).toHaveBeenCalledWith({ filters: {} }, 'key', expect.any(Function))
  })

  it('computes KPI cards with ratio', () => {
    const store = useAnalyticsStore()
    store.costBenefitResult = {
      summary: { total_cost: 100, total_revenue: 250, net_profit: 150 },
      items: [],
      metrics: [],
    }
    const ratioCard = store.kpiCards.find((card) => card.label === '成本收益比')
    expect(ratioCard?.value).toBeCloseTo(2.5)
  })
})
