...
export const useAnalyticsStore = defineStore('analytics', () => {
  ...
  const kpiCards = computed(() => {
    ...
    return [
      { label: '總成本', value: totalCost, unit: 'TWD', type: 'currency' },
      { label: '總收益', value: totalRevenue, unit: 'TWD', type: 'currency' },
      { label: '淨收益', value: netProfit, unit: 'TWD', type: 'currency' },
      { label: '成本收益比', value: ratio, unit: '', type: 'ratio' },
    ]
  })
  ...
  const hasCostEntries = computed(() => Array.isArray(costEntries.value) && costEntries.value.length > 0)
  const hasRevenueEntries = computed(() => Array.isArray(revenueEntries.value) && revenueEntries.value.length > 0)
  ...
  const generateReport = async (payload, apiKey) => {
    loading.report = true
    error.value = null
    try {
