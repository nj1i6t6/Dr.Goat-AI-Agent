...
from app.utils import call_gemini_api, stream_gemini_api, get_sheep_info_for_context, encode_image_to_base64
...
def get_agent_tip():
    ...
    result = call_gemini_api(
        prompt,
        api_key=api_key,
        generation_config_override={"temperature": 0.7},
        timeout_seconds=45,
    )
    if "error" in result:
...
def get_recommendation():
    ...
    try:
        result = call_gemini_api(
            full_prompt,
            api_key=api_key,
            generation_config_override={"temperature": 0.55},
            timeout_seconds=70,
        )
    except Exception as e:
        current_app.logger.error(f"呼叫 Gemini API 發生例外: {e}", exc_info=True)
        result = {"error": f"AI 服務暫時不可用: {e}"}

    if "error" in result:
        # 回傳簡短 fallback 內容，避免 504
        fallback_html = markdown.markdown(
            "目前無法取得完整 AI 建議，請稍後再試。您仍可檢查輸入欄位是否完整，或確認 API Key 是否有效。",
            extensions=['nl2br']
        )
        return jsonify(error=result["error"], recommendation_html=fallback_html), 500
...
@bp.route('/recommendation/stream', methods=['POST'])
@login_required
def stream_recommendation():
    """SSE 版飼養建議（暫用非真正串流的分塊輸出，保留相容性）。"""
    # 重用邏輯組裝 prompt
    try:
        recommendation_data = AgentRecommendationModel(**request.get_json())
    except ValidationError as e:
    ...
    def generate():
        try:
            with current_app.app_context():
                # 在 app context 內進行需要的查詢/當前使用者資訊存取（此路由目前未查 DB，但保留模式）
                pass
            for piece in stream_gemini_api(full_prompt, api_key=api_key, generation_config_override={"temperature": 0.55}):
                yield f"data: {piece}\n\n"
            yield "event: done\ndata: [DONE]\n\n"
        except Exception as e:
            # 回退：若真串流失敗，回到一次性請求避免整體中斷
            res = call_gemini_api(full_prompt, api_key=api_key, generation_config_override={"temperature": 0.55})
            if "error" in res:
                yield f"data: {res['error']}\n\n"
            else:
                text = res.get("text", "")
                for i in range(0, len(text), 200):
                    yield f"data: {text[i:i+200]}\n\n"
                yield "event: done\ndata: [DONE]\n\n"
...
# Alias: /api/nutrition_advice 與 /api/nutrition_advice/stream
@bp.route('/nutrition_advice', methods=['POST'])
@login_required
def nutrition_advice_alias():
    return get_recommendation()


@bp.route('/nutrition_advice/stream', methods=['POST'])
@login_required
def nutrition_advice_stream_alias():
...
def chat_with_agent_stream():
    ...
    # 捕捉 request 期間的 user_id，避免在背景串流中取用 current_user
    user_id = current_user.id
    ...
    def generate():
        try:
            with current_app.app_context():
                ...
                for entry in history:
                    chat_messages_for_api.append({"role": entry.role, "parts": [{"text": entry.content}]})
                chat_messages_for_api.append({"role": "user", "parts": [{"text": user_message}]})
            for piece in stream_gemini_api(chat_messages_for_api, api_key=api_key, generation_config_override={"temperature": 0.7}):
                yield f"data: {piece}\n\n"
            yield "event: done\ndata: [DONE]\n\n"
        except Exception:
            res = call_gemini_api(chat_messages_for_api, api_key=api_key, generation_config_override={"temperature": 0.7})
            if "error" in res:
                yield f"data: {res['error']}\n\n"
            else:
                text = res.get("text", "")
                for i in range(0, len(text), 160):
                    yield f"data: {text[i:i+160]}\n\n"
                yield "event: done\ndata: [DONE]\n\n"
